...
def populate_master_load(conn, load, load_type):
    """
    """
    project = (load.name, load.title, load_type)
    sql = 'INSERT INTO tb_LoadBasic(name, title, type) \
           VALUES(?,?,?)'
    cur = conn.cursor()
    cur.execute(sql, project)
    return cur.lastrowid
#
def populate_node_load_table(conn, load_number, node):
    """
    ...
    if node.system != 0:
       system = "local" 
    #
    project = (load_number, node.load_name,
               node.number, system, node.load_complex,
               node.fx, node.fy, node.fz, node.mx, node.my, node.mz,
               'NULL', 'NULL', 'NULL', 'NULL', 'NULL', 'NULL')
    #
    sql = 'INSERT INTO tb_LoadNode(load_number, load_title,\
    ...
    cur.execute(sql, project)
#
def populate_beam_line_load_table(conn, load_number, memb):
    """
    Create a new project into the projects table
    :param conn:
    :param project:
    ...
    if memb.system != 0:
       system = "local" 
    #
    project = (load_number, memb.load_name,
               memb.number, system, memb.load_complex,
               memb.L1, memb.qx1, memb.qy1, memb.qz1,
               'NULL', 'NULL', 'NULL',
               memb.L2, memb.qx2, memb.qy2, memb.qz2,
               'NULL', 'NULL', 'NULL')
    #
    sql = 'INSERT INTO tb_LoadBeamLine(load_number, load_title,\
    ...
    cur.execute(sql, project)
#
def populate_beam_point_load_table(conn, load_number, memb):
    """
    Create a new project into the projects table
    :param conn:
    :param project:
    ...
    project = (load_number, memb.load_name,
               ...
               'NULL', 'NULL', 'NULL')
    #
    sql = 'INSERT INTO tb_LoadBeamPoint(load_number, load_title,\
                                          element_name, system, load_complex,\
...
def populate_basic_loading(conn, basic_load):
    ...
    for load_name, load_basic in basic_load.items():
        load_number = populate_master_load(conn, load_basic, load_type)
    #
    # node
    #create_table(conn, _table_node_load)
    #for load_name, load_basic in basic_load.items():
        for basic in load_basic.point_node.values():
            for node in basic:
                populate_node_load_table(conn, load_number, node)
    #
    # beam element
    #create_table(conn, _table_element_line_load)
    #create_table(conn, _table_element_point_load)
    #for load_name, load_basic in basic_load.items():
        for basic in load_basic.udl_beam.values():
            for element in basic:
                populate_beam_line_load_table(conn, load_number,  element)
        for basic in load_basic.point_beam.values():
            for element in basic:
                populate_beam_point_load_table(conn,load_number, element)
    #
    conn.commit()
    #print('-->')