...
def get_force(conn, node_name: int|str|None=None,
    ...
    if node_name:
        #project = (node_name,)
        query = f'SELECT {item} FROM tb_Fn WHERE node_name = {node_name}'
    else:
        query = f'SELECT {item} FROM tb_Fn'
...
def get_displacement(conn, node_name: int|str|None=None,
    ...
    if node_name:
        #project = (node_name,)
        query = f'SELECT {item} FROM tb_Un WHERE node_name = {node_name}'
    else:
        query = f'SELECT {item} FROM tb_Un'
...
def get_reactions(conn, node_name: int|str|None=None,
    ...
    if node_name:
        #project = (node_name,)
        query = f'SELECT {item} FROM tb_Rn WHERE node_name = {node_name}'
    else:
        query = f'SELECT {item} FROM tb_Rn'
...
def print_nitems(items,
    ...
    for key, wk in nditems:
        header1 =  wk[['load_name','load_system']].values
        #
        output += "-- Basic Load  Name: {:}  System: {:}\n".format(*header1[0])
        #
    ...
    try:
        ...
        for key, wk in nditems:
            ...
            output += "-- Load Combination  Name: {:}  System: {:}\n".format(*header1[0])
            #output += header2
            #
            vals = wk.drop(cols, axis=1)
            header2 = list(vals)
