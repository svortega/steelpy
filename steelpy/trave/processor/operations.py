...
class ElementProcess:
    __slots__ = ['_plane', '_elements', '_load', 
                 '_boundaries', '_ndisp']
    
    def __init__(self, elements, boundaries,
    ...
    def solve(self, df_ndisp, steps: int):
        ...
        bload_func = basic_load.process(elements=self._elements,
                                        steps=steps)
        #
        df_beamf, df_nforce = self.solve_forces(bload_func, df_ndisp,
                                                df_nload, steps)

        return df_beamf, df_nforce
    #
    #
    def solve_forces(self, bload_func,
    ...
    # -----------------------------------------------------------
    #
    def internal_force(self, beam_steps:int = 10):
        """
        beam_steps : Integration points beam element (10 default)
        """
        df_ndisp = self._ndisp
        df_beamf, df_nforce = self.solve(df_ndisp, beam_steps)
        #self._node_force = df_nforce
        #
        # combination
        load_comb = self._load.combination()
        df_comb = load_comb.to_basic()
        #
        df_beam = update_memberdf2(dfmemb=df_beamf,
                                   dfcomb=df_comb,
        ...
        #df_ndisp.set_index(header)
        #df_nforce.set_index(header)
        #
        #df_ndisp.update(df_nforce)
        #
        df_nres = df_ndisp.set_index(header).join(df_nforce.set_index(header))
        
        #df_ndisp.join(df_nforce, on=['load_name', 'load_number', 'load_type',
        #                             'load_system', 'load_title', 'node_name'])
        #
        return Results(noderes=df_nres.reset_index(), 
                       nodereac=df_nreac,
                       beamres=df_beam,
                       plane=self._plane)
    #
