...
#
class ElementDirectionCosine(Mapping):
    """
    ...
    **Parameters**:  
    ...
    def __setitem__(self, element_id: int, value: List) -> None:
        """
        """
        try:
            self._labels[element_id]
            print('    *** warning Direction Cosines {:} already exist'.format(element_id))
            1 / 0
            return
        except IndexError:
            self._labels.append(element_id)
            #_number = max(self._number) + 1
            self._number.append(element_id)
            if len(value) == 3:
                self._type.append(3)
    ...
    def __getitem__(self, element_id: int) -> Tuple:
        """
        """
        try:
            _index = self._labels.index(element_id)
    ...
    def __delitem__(self, element_id: int) -> None:
        ...
        """
        try:
            i = self._labels.index(element_id)
            self._labels.remove(element_id)
            self._number.pop(i)
            self._type.pop(i)
            #
            self._c11.pop(i)
