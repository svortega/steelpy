...
class BeamConcept(Mapping):
    ...
    **Parameters**:  
    ...
    #def __delitem__(self, element_id: int) -> None:
    #    """
    #    """
    #    del self._distributed[element_id]
    #    del self._mass[element_id]
#
#
#
class BeamProperties:
    ...
    def __getattr__(self, value):
        ...
        if value.lower() in ['cdcm']:
            ...
            _element_id = self._cls._properties.hydrodynamic.CdCm._cdcm.get_item_name(_item_number)
            return self._cls._properties.hydrodynamic.CdCm[_element_id]
        
        elif value.lower() in ['flooded']:
            _item_number = self._cls._flooded[self._element_name]
            _element_id = self._cls._properties.hydrodynamic._flooded.get_item_name(_item_number)
            return self._cls._flooded[_element_id]
        
        elif value.lower() in ['marine_growth']:
            _item_number = self._cls._marine_growth[self._element_name]
            _element_id = self._cls._properties.hydrodynamic._marine_growth.get_item_name(_item_number)            
            return self._cls._marine_growth[_element_id]
        
        elif value.lower() in ['diameter']:
            _item_number = self._cls._hydro_diameter[self._element_name]
            _element_id = self._cls._properties.hydrodynamic._hydro_diameter.get_item_name(_item_number)            
            return self._cls._hydro_diameter[_element_id]
    ...
    def __setattr__(self, key, value):
        ...
        if key.lower() in ['cdcm']:
            ...
            #_item.sets.elements.append(self.element_id)
