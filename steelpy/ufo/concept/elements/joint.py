...
class Connection(Mapping):
    ...
    #
    #
    def map_joint_with_node(self, joint_name: str, node_id: int) -> None:
        """
        """
        try:
    ...
    def __delitem__(self, joint_name) -> None:
        ...
        if 'link' in self._joints[joint_name].type:
            try:
                _node_id = self._joints[joint_name].node.number
                del self._boundaries[_node_id]
            except KeyError:
                pass
    ...
    **Parameters**:  
    ...
    def node(self, node_id: int)-> None:
        ...
        try:
            ...
            raise(f'   *** error : Node {node_id} not found')
    #
    @property
    def boundary(self):
        """
...
class Points(Mapping):
    ...
    def __setitem__(self, point_name: str, coordinates:tuple) -> None:
        """
        """
        node_id = len(self._points) + 1
        try:
            self.map_point_with_node(point_name, node_id)
        except KeyError:
    ...
    #
    #
    def map_point_with_node(self, point_name: str, node_id: int) -> None:
        """
        """
        try:
            ...
            try:
                ...
                self._points[point_name].node = node_id
            except IndexError:
